...
const PaySlip = () => {
  ...
  const grossSalary =
    ...
    Number(employee.empPFF || 0) +
    Number(employee.employerPFF || 0) +
    Number(employee.otherAllowance || 0);

  const totalDeductions =
    Number(employee.empPF || 0) +
    Number(employee.employerPF || 0) +
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <div style={{ backgroundColor: "#ffffff" }} className="p-6 rounded-lg shadow-md max-w-6xl mx-auto ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          {["ctc", "basic", "da", "hra", "empPFF", "employerPFF", "otherAllowance"].map((key) => (
            <div key={key} className="flex flex-col">
              <label style={{ color: "#374151" }} className="block font-medium mb-1 capitalize">
                {key.replace(/([A-Z])/g, " $1")}
              </label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {["empPF", "employerPF", "otherDeduction"].map((key) => (
            <div key={key} className="flex flex-col">
              <label style={{ color: "#374151" }} className="block font-medium mb-1 capitalize">
                {key.replace(/([A-Z])/g, " $1")}
              </label>
